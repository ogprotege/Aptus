...
Inspect CUDA hardware on the server host:
...
plan, and transforms every canonical row.
The preflight uses the selected method with synthetic tensors. The pilot uses a
bounded real model and data pressure set in two fresh processes and confirms
