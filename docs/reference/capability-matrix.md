...
| `mlx-lm` | Exact external Python executable, including persisted Mac selection | Single-device LoRA and QLoRA, including the conditional Qwen3 MoE row, reviewed 24-layer dense Qwen2 footprint, dense Gemma 4, unified Gemma 4 (Exit B / compiler-contract unsupported), and conditional `gemma4_moe` as planner/compiler identities | Two current v5/v3 dense QLoRA workflows reached `measured-run-pass` for the exact accepted Qwen2.5 artifact; that result does not transfer to dense Gemma 4, unified Exit B, or `gemma4_moe`. Every different Qwen2 artifact and the Qwen3 MoE row remain conditional and pilot-required. `gemma4_moe` has no measured ladder |
