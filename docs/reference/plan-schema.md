...
configuration footprint, not an artifact allowlist. A path emitted in the plan
and its compiler binding establish conditional eligibility, not runtime proof.
...
That result supplies current-contract Phase 6 runtime evidence at that
acceptance source only for that scope. A different matching artifact still
requires its own model-data, measured-preflight, and pilot gates; the result
does not qualify CUDA or establish safety, model quality, performance, production throughput,
...
qualifies one exact SmolLM2 execution only; it neither qualifies the MLX result
for CUDA nor changes this plan schema.
