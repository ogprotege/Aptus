...
The repository tree contains 128 tracked Markdown documents. Of those, 127 are
...
a separate active packet without overwriting the earlier cohort history.

| Lifecycle | Markdown files | Meaning |
|---|---:|---|
| Active | 98 | Current guidance, governance, navigation, evidence, protocol, or current research with explicit limits |
...
| Total | 127 | Excludes only the pull-request workflow template from tracked Markdown governance |
...
The automated `maintained_documentation()` set contains the same 127 governed
...
The single tracked Markdown file outside the 127-file lifecycle count is
